...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Improved to handle multi-button forms where button name/value is needed
    const forms = document.querySelectorAll('form');
    forms.forEach(form => {
        form.addEventListener('submit', function (e) {
            // Find the button that was clicked to submit the form
            // This is usually activeElement if the submission was triggered by a click
            const clickedBtn = document.activeElement;
            const submitBtn = (clickedBtn && clickedBtn.type === 'submit' && clickedBtn.form === form)
                ? clickedBtn
                : form.querySelector('button[type="submit"]');

            if (submitBtn && !submitBtn.disabled) {
                // Use setTimeout to allow the browser to capture the button name/value 
                // for the request before we disable it.
