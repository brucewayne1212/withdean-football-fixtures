...
def refresh_fixtures():
    ...
    try:
        ...
        # Filter out past fixtures (older than today)
        # This helps ignore "hidden" rows which often contain previous matches
        current_date = datetime.now().date()
        filtered_fixtures = []
        for fixture in fixtures_data:
            date_str = fixture.get('date')
            # parse_flexible_date returns a datetime with UTC tz or None
            ...
            if parsed_date:
                ...
                # Keep unparseable dates to be safe (user can uncheck them)
                filtered_fixtures.append(fixture)
                
        fixtures_data = filtered_fixtures
        ...
        # Get managed teams for filtering
        managed_teams = session.query(Team).filter_by(
            organization_id=org.id,
            is_managed=True
        ).all()
...
def save_refreshed_fixtures():
    ...
    try:
        ...
        for i in range(max_index + 1):
            ...
            import_fixture = request.form.get(f'fixture_{i}_import')
            
            if not fixture_json or not import_fixture:
                continue
