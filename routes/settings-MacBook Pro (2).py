...
# Team management routes moved to routes/teams.py
...
def settings_view():
    ...
    try:
        ...
        # Mock user_manager for template compatibility
        ...
        # Get email templates
        email_templates = session.query(EmailTemplate).filter_by(
            organization_id=org.id
        ).order_by(EmailTemplate.template_type, EmailTemplate.name).all()
        
        active_default = next((t for t in email_templates if t.template_type == 'default' and t.is_active), None)
        
        return render_template('settings.html',
            user_info={
                'name': current_user.name, 
                'email': current_user.email,
                'created_date': current_user.created_at.isoformat() if hasattr(current_user, 'created_at') and current_user.created_at else '',
            ...
            email_template=active_default.content if active_default else '',
...
# Coach management routes moved to routes/teams.py
...
# Kit management routes moved to routes/teams.py
...
# FA URL management routes moved to routes/teams.py

@settings_bp.route('/manage_email_template', methods=['GET', 'POST'], strict_slashes=False)
@settings_bp.route('/manage_email_template/<template_id>', methods=['GET', 'POST'], strict_slashes=False)
@login_required
def manage_email_template(template_id=None):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # Get template_id from form if not in URL, and handle 'None' string
            if not template_id or str(template_id).lower() == 'none':
                template_id = request.form.get('template_id')
            
            if str(template_id).lower() == 'none':
                template_id = None

            if action == 'save':
                template_content = request.form.get('template_content', '')
                template_name = request.form.get('template_name', '').strip() or 'Default Template'
                is_default_target = request.form.get('is_default') == 'true'
                
                logger.info(f"EMAIL_TEMPLATE_SAVE: Name='{template_name}', Default={is_default_target}, Content length={len(template_content)}")
                
                # Find template to update
                email_template = None
                if template_id and str(template_id).lower() != 'none':
                    try:
                        from uuid import UUID
                        search_id = UUID(str(template_id)) if isinstance(template_id, str) else template_id
                        email_template = session.query(EmailTemplate).filter_by(id=search_id, organization_id=org_id).first()
                        if email_template:
                            logger.info(f"EMAIL_TEMPLATE_SAVE: Found existing template by ID: {email_template.id}")
                    except Exception as e:
                        logger.error(f"EMAIL_TEMPLATE_SAVE: ID lookup error: {e}")

                # If not found by ID, and it's marked as default, try to find the organization's default
                if not email_template and is_default_target:
                    logger.info("EMAIL_TEMPLATE_SAVE: No specific ID found, looking for existing default for org.")
                ...
                if email_template:
                    logger.info(f"EMAIL_TEMPLATE_SAVE: UPDATING template ID {email_template.id}")
                    email_template.content = template_content
                    email_template.name = template_name
                    email_template.template_type = 'default' if is_default_target else 'custom'
                    email_template.updated_at = func.now()
                else:
                    # Check if a template with this name already exists to avoid UniqueConstraint error
                    existing_name = session.query(EmailTemplate).filter_by(
                        organization_id=org_id,
                        name=template_name
                    ).first()
                    
                    if existing_name:
                        logger.info(f"EMAIL_TEMPLATE_SAVE: Found existing template by name '{template_name}', updating it instead of creating new.")
                        email_template = existing_name
                        email_template.content = template_content
                        email_template.template_type = 'default' if is_default_target else 'custom'
                        email_template.updated_at = func.now()
                    else:
                        logger.info("EMAIL_TEMPLATE_SAVE: CREATING NEW template record.")
                        email_template = EmailTemplate(
                            organization_id=org_id,
                            name=template_name,
                            template_type='default' if is_default_target else 'custom',
                ...
                try:
                    session.flush()
                    # If this template is now default, deactivate all others
                    if email_template.template_type == 'default':
                        session.query(EmailTemplate).filter(
                            EmailTemplate.organization_id == org_id,
                            EmailTemplate.template_type == 'default',
                            EmailTemplate.id != email_template.id
                        ).update({EmailTemplate.is_active: False})

                    session.commit()
                    logger.info(f"EMAIL_TEMPLATE_SAVE: SUCCESS! Committed ID {email_template.id}")
                    flash(f'Template "{template_name}" saved successfully.', 'success')
                    return redirect(url_for('settings.manage_email_template', template_id=email_template.id))
                except Exception as e:
                    session.rollback()
                    logger.error(f"EMAIL_TEMPLATE_SAVE: COMMIT FAILED: {e}")
                    import traceback
                    logger.error(traceback.format_exc())
                    flash(f'Database error: {str(e)}', 'error')
                    return redirect(url_for('settings.manage_email_template', template_id=template_id))

            elif action == 'reset':
                # Reset only makes sense for the default template or deleting current
                if template_id and template_id != 'None':
                    try:
                        from uuid import UUID
                        search_id = UUID(template_id) if isinstance(template_id, str) else template_id
                        email_template = session.query(EmailTemplate).filter_by(
                            id=search_id,
                            organization_id=org_id
                        ).first()
                        if email_template:
                            ...
                            session.commit()
                            flash(f'Template "{name}" has been deleted/reset.', 'success')
                        else:
                            flash('Template not found.', 'error')
                    except Exception as e:
                        logger.error(f"EMAIL_TEMPLATE_RESET: Error: {e}")
                        flash(f'Error: {e}', 'error')
                else:
                    # Legacy reset behavior - reset the default template
                    email_template = session.query(EmailTemplate).filter_by(
                        organization_id=org_id,
                        template_type='default'
                    ).first()
                    if email_template:
                        session.delete(email_template)
                        session.commit()
                    flash('Default email template reset.', 'success')
        ...
        if template_id and template_id != 'None':
            try:
                from uuid import UUID
                search_id = UUID(template_id) if isinstance(template_id, str) else template_id
                template_to_edit = session.query(EmailTemplate).filter_by(
                    id=search_id,
                    organization_id=org_id
                ).first()
                if template_to_edit:
                    logger.info(f"EMAIL_TEMPLATE_GET: Found template ID {template_to_edit.id} ('{template_to_edit.name}')")
            except Exception as e:
                logger.error(f"EMAIL_TEMPLATE_GET: Error fetching template {template_id}: {e}")
        
        # If no specific ID found or provided, try to find the active default
        if not template_to_edit:
            template_to_edit = session.query(EmailTemplate).filter_by(
                organization_id=org_id,
                template_type='default'
            ).order_by(EmailTemplate.is_active.desc(), EmailTemplate.updated_at.desc()).first()
            if template_to_edit:
                logger.info(f"EMAIL_TEMPLATE_GET: Falling back to default template ID {template_to_edit.id}")
        ...
        # Handle "Add New" feature with pre-population
        if request.args.get('new') == 'true':
            copy_from_id = request.args.get('copy_from')
            if copy_from_id:
                source = session.query(EmailTemplate).filter_by(id=copy_from_id, organization_id=org_id).first()
                template_content = source.content if source else get_default_email_template()
            else:
                # Default to current active default or hardcoded default
                template_content = template_to_edit.content if template_to_edit else get_default_email_template()
        ...
        # Normal Edit Mode
        template_content = template_to_edit.content if template_to_edit else get_default_email_template()
        template_name = template_to_edit.name if template_to_edit else "Default Template"
        is_default = (template_to_edit.template_type == 'default') if template_to_edit else True

        return render_template('email_template_editor.html',
                             template_id=template_id or (str(template_to_edit.id) if template_to_edit else None),
                             template_content=template_content,
        ...
        import traceback
        print(f"ERROR in manage_email_template: {str(e)}")
